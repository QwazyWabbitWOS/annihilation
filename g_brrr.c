...
void X_Radio_f(edict_t* self, char channel, char* msg, char* person)
{
	...
	if (strlen(temp1->string) > 0)
		Com_sprintf(cmd, sizeof cmd, "./%s/sound/radio/%s.wav", temp1->string, msg);
	else
		Com_sprintf(cmd, sizeof cmd, "./sound/radio/%s.wav", msg);
	...
	Q_strupper(msg);
	sprintf(cmd, "echo %s message from %.16s - [%.20s]...;play radio/%s\n", temp, self->client->pers.netname, msg, msg);   // Build command to send
...
void PipeBomb_Touch(edict_t* ent, edict_t* other, cplane_t* plane, csurface_t* surf)
{
	...
	if (ctf->value && other->client && other->client->resp.ctf_team == ent->owner->client->resp.ctf_team)
		return;
...
/* *** FLARES *** */
void killflare(edict_t* self, edict_t* inflictor, edict_t* attacker, int damage, vec3_t point)
{
	self->takedamage = DAMAGE_NO;
	self->die = nothing;
...
void Use_BFGArmor(edict_t* self, gitem_t* item)
{
	edict_t* bfg;
...
void Cmd_Airstrike_f(edict_t* ent)
{
	...
	if (ent->client->airstrike_called)
	{
		...
		// kill the marker
		ent->parent->nextthink = FRAMETIME;
...
void nothing(edict_t* self, edict_t* inflictor, edict_t* attacker, int damage, vec3_t point)
{
	return;
}
...
void brrr_memos(edict_t* ent)
{
	...
	Q_strncatz(filename, sizeof filename, game_dir->string);
	Q_strncatz(filename, sizeof filename, "/memos.txt");
...
qboolean saveitemdata(edict_t* ent)
{
	...
	Q_strncpyz(filename, sizeof filename, "./");
	Q_strncatz(filename, sizeof filename, game_dir->string);
	Q_strncatz(filename, sizeof filename, "/config/");
	Q_strncatz(filename, sizeof filename, level.mapname);
	Q_strncatz(filename, sizeof filename, ".ann");
...
qboolean loaditemdata(void)
{
	...
	while (!feof(fp))
	{
		...
		if (fgets(buffer, 256, fp) == NULL)
			gi.dprintf("Error reading %s in %s, %s\n", filename, __func__, strerror(errno));
		// Remove trailing '\n'
		...
		{
			classname[0] = '\0';
			if (sscanf(buffer, "%d %d %d %63s", &a, &b, &c, classname) != 4) {
				gi.dprintf("Warning: item initialization incomplete, %s in %s\n", classname, __func__);
			}
			spot[0] = (float)a;
			spot[1] = (float)b;
			spot[2] = (float)c;
...
void Cmd_Convert_f(edict_t* ent, char* arg1, char* arg2)
{
	...
	{
		if (currency && ent->client->resp.conv_from)
		{
...
void convert(edict_t* ent)
{
	...
	if (!currency1 || (*currency1) + ent->client->resp.conv_paid < exchange2)
		return;
...
void radar_update(edict_t* ent)
{
	...
	//QW// There is no itoa function in the standard C library so we use sprintf instead.
	char num[16] = { 0 };
	sprintf(num, "%i", ent->client->resp.radar_zoom);

	sprintf(string, "xv 80 yv 40 picn rpic xv 240 yv 200 strin%s \"%s\" ", (ent->client->radarcol = !ent->client->radarcol) ? "g" : "g2", num /* itoa(ent->client->resp.radar_zoom, type, 10 */);
	...
	Q_strncpyz(string2, sizeof string2, string);
...
void InitRotate(void)
{
	...
	Q_strncatz(filename, sizeof filename, rotate_filename->string);
	...
	maplist.mapname = gi.TagMalloc((int)(MAX_QPATH * sizeof(char) * line), TAG_GAME);
	...
	maplist.mapused = gi.TagMalloc((int)(sizeof(qboolean) * line), TAG_GAME);
	...
	while (!feof(fp))
	{
		...
		Q_strncpyz(&maplist.mapname[line * MAX_QPATH * sizeof(char)], MAX_QPATH, buffer);
...
void listmaps(void)
{
	...
	for (n = 0; n < maplist.maxindex; n++)
		gi.cprintf(NULL, PRINT_HIGH, "   %d   %s\n", maplist.mapused[n], &maplist.mapname[n * MAX_QPATH * (int)sizeof(char)]);
...
edict_t* maprotate(qboolean reset)
{
	...
	ent->map = &maplist.mapname[maplist.mapindex * MAX_QPATH * (int)sizeof(char)];

	gi.cprintf(NULL, PRINT_HIGH, "Rotating to %s.map ...\n", &maplist.mapname[maplist.mapindex * MAX_QPATH * (int)sizeof(char)]);
