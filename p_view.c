...
void SV_CalcBlend(edict_t* ent)
{
	...
	{
		SV_AddBlend(1, 1, 1, 0.9f, ent->client->ps.blend);
	...
	if (ent->client->goggles)
		SV_AddBlend(-0.5f, -1, -1, 0.5f, ent->client->ps.blend);
	/* *** */

	/* *** CLOAKING *** */
	if (ent->client->cloakable == 1 && (ent->svflags & SVF_NOCLIENT))
		SV_AddBlend(-1, -1, -1, 0.3f, ent->client->ps.blend);
	/* *** */

	/* *** CLOAKING / CLOAKING JAMMER *** */
	if (ent->client->cloaking == 2)
		SV_AddBlend(1, 0, 1, 0.3f, ent->client->ps.blend);
	/* *** */

	/* *** BFG ARMOUR *** */
	if (ent->client->bfg_blend)
		SV_AddBlend(0, 1, 0, 0.3f, ent->client->ps.blend);
	/* *** */
	...
	if (Jet_Active(ent))
	{
		...
		if (((int)ent->client->Jet_remaining % 6) == 0)
			gi.sound(ent, CHAN_AUTO, gi.soundindex("hover/hovidle1.wav"), 0.9f, ATTN_NORM, 0);
	...
	/* *** KAMIKAZE MODE *** */
			...
			if (remaining > 30 || (remaining & 4))
				SV_AddBlend(1, 0, 0, 0.08f, ent->client->ps.blend);
		}
	/* *** */
	/* *** FREEZER *** */
		else if (ent->client->frozen)
		{
			SV_AddBlend(0, 0, 1, 0.20f, ent->client->ps.blend);
		}
	/* *** */
